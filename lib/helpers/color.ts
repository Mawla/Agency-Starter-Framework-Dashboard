...
export const lighten = (hex: string): string => {
  const newVal = hsvaToHsla(hexToHsva(hex));
  newVal.l = 90;
  return hsvaToHex(hslaToHsva(newVal));
};

export const darken = (hex: string): string => {
  const newVal = hsvaToHsla(hexToHsva(hex));
  newVal.l = 20;
  return hsvaToHex(hslaToHsva(newVal));
};
...
export const contrast = (hex: string): string => {
  let newVal = hsvaToHsla(hexToHsva(hex));
  newVal.l = contrastColor({ bgColor: hex }) === "#FFFFFF" ? 99 : 1;
  return hsvaToHex(hslaToHsva(newVal));
};
